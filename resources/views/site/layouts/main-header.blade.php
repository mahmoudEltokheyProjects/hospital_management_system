    ...
    <div class="container-fluid">
        <div class="main-header-left ">
            <div class="responsive-logo">
                ...
                <a href="{{ url('/' . ($page = 'index')) }}">
                    <img src="{{ URL::asset('assets/img/brand/favicon.png') }}" class="logo-2" alt="logo"></a>
                <a href="{{ url('/' . ($page = 'index')) }}">
                    <img src="{{ URL::asset('assets/img/brand/favicon.png') }}" class="dark-logo-2" alt="logo">
                </a>
            </div>
        ...
        <div class="main-header-right">
                ...
                <li class="">
                   {{-- +++++++++++++++++++++++++++ start : Language Switcher +++++++++++++++++++++++++++ --}}
                    <div class="btn-group mb-1">
                        <button type="button" class="btn btn-light btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="currentLanguage">
                            @if (App::getLocale() == 'ar')
                                العربية
                                &nbsp;&nbsp;<img src="{{ URL::asset('assets/img/flags/EG.png') }}" alt="Arabic" class="ml-2" id="currentFlag">
                            @else
                                English
                                &nbsp;&nbsp;<img src="{{ URL::asset('assets/img/flags/US.png') }}" alt="English" class="ml-2" id="currentFlag">
                            @endif
                        </button>
                        <div class="dropdown-menu">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                <a class="dropdown-item d-flex align-items-center switch-language"
                                   data-locale="{{ $localeCode }}"
                                   data-flag="{{ URL::asset('assets/img/flags/' . ($localeCode == 'ar' ? 'EG' : 'US') . '.png') }}"
                                   rel="alternate" hreflang="{{ $localeCode }}"
                                   href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}">
                                    <img src="{{ URL::asset('assets/img/flags/' . ($localeCode == 'ar' ? 'EG' : 'US') . '.png') }}"
                                         alt="{{ $properties['native'] }}" class="mr-2" width="20">&nbsp;&nbsp;&nbsp;
                                    {{ $properties['native'] }}
                                </a>
                            @endforeach
                        </div>
            ...
            <div class="nav nav-item  navbar-nav-right ml-auto">
                ...
                <!-- Appear "Notifications" Only For Users have "permission" = "الاشعارات"  -->
                @can('الاشعارات')
                    <div class="dropdown nav-item main-header-notification">
                        <a class="new nav-link" href="#">
                            ...
                            <!-- Appear "green circle" of "notification" if There are any "new Added Invoice" -->
                        ...
                        <div class="dropdown-menu">
                            <div class="menu-header-content bg-primary text-right">
                                <div class="d-flex">
                                    ...
                                    <a href="{{ route('MarkAsRead_all') }}" class="mr-auto my-auto float-left">
                                        <span class="badge badge-pill badge-warning">
                                        تعين قراءة الكل
                                        </span>
                                    </a>
                                </div>
                                <p class="dropdown-title-text subtext mb-0 text-white op-10 pb-0 tx-12" id="notifications_count">
                                    عدد الاشعارات الغير مقروءة :
                                    <span style="color:yellow">{{ auth()->user()->unreadNotifications->count() }} </span>
                                </p>
                            </div>
                            <!-- +++++++++++++ unreadNotifications +++++++++++++  -->
                            <div id="unreadNotifications">
                                @foreach ( auth()->user()->unreadNotifications as $notificationVar )
                                <div class="main-notification-list Notification-scroll">
                                    <a class="d-flex p-3 border-bottom" href="{{ url('InvoicesDetails') }}/{{$notificationVar->data['id']}}">
                                        <div class="notifyimg bg-pink">
                                            <i class="la la-file-alt text-white"></i>
                                        </div>
                                        <div class="mr-3">
                                            <h5 class="notification-label mb-1">
                                                {{ $notificationVar->data['title'] }} {{ $notificationVar->data['user_create_invoice'] }}
                                            </h5>
                                            <div class="notification-subtext">
                                                {{ $notificationVar->created_at }}
                                            </div>
                                        </div>
                                        <div class="mr-auto">
